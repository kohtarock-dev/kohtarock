...
- **skymtackle**(BASEショップ。「放流」カテゴリページを直接指定してポーリング)
...
robots.txt自体の取得も、実際のページ取得と同じ名前を名乗ったUser-Agentで行います
(標準ライブラリの無名なUser-Agentだとrobots.txtの取得自体をWAFに拒否されるサイトが
実際にあったため)。取得に失敗した場合は安全側に倒してブロック扱いにします。
